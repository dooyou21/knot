...
function elem(seq, pos) {
  ...
  if (typeof seq == 'string') return seq.charAt(pos);
  // is an array
  else return seq[pos];
}
...
export function PATCH() {
  ...
  if (!Array.isArray(this.hunks)) throw new Error('Invaid Argument');
  this.hunks.forEach(function (hunk) {
    ...
    if (typeof hunk.op.get_length_change != 'function')
      throw new Error(
        'Invalid Argument (hunk operation ' +
          hunk.op.inspect() +
          ' does not support get_length_change)',
      );
    if (typeof hunk.op.decompose != 'function')
      throw new Error(
        'Invalid Argument (hunk operation ' +
          hunk.op.inspect() +
          ' does not support decompose)',
      );
    Object.freeze(hunk);
  });
...
export function ATINDEX() {
  ...
  if (arguments.length == 1) {
    ...
    op_map = arguments[0];
    indexes = [];
    Object.keys(op_map).forEach(function (index) {
...
PATCH.prototype.simplify = function () {
  ...
  function handle_hunk(hunk) {
    ...
    if (op.isNoOp()) {
      ...
      hunk.length == 0 &&
      hunk.op.get_length_change(hunk.length) == 0
    ) {
      // The hunk does nothing. Drop it, but adjust future offsets.
      doffset += hunk.offset;
      return;
    } else if (hunks.length > 0 && hunk.offset == 0 && doffset == 0) {
  ...
  function make_state(op, side) {
    return {
      ...
      start: function () {
        ...
        if (side == 0) ret += h.op.get_length_change(h.length);
        return ret;
      },
    };
  }
...
function rebase_patches(a, b, conflictless) {
  ...
  function make_state(op) {
    return {
      ...
      new_hunks: [],
      empty: function () {
        return this.old_hunks.length == 0;
      },
  ...
  while (!a_state.empty() || !b_state.empty()) {
    ...
    if (
      ...
      if (jot.cmp(a_state.old_hunks[0].op, b_state.old_hunks[0].op) == 0) {
        ...
        jot.cmp(a_state.old_hunks[0].op, b_state.old_hunks[0].op) < 0
      ) {
        a_state.take(b_state);
      } else {
        b_state.take(a_state);
    ...
    if (dx_start == 0 && dx_end == 0) {
      ...
      var conflictless2 = !conflictless ? null : clone(conflictless);
      if (conflictless2 && 'document' in conflictless2)
        conflictless2.document = conflictless2.document.slice(
          a_state.start(),
          a_state.end(),
        );

      var ar = a_state.old_hunks[0].op.rebase(
        b_state.old_hunks[0].op,
        conflictless2,
      );
      var br = b_state.old_hunks[0].op.rebase(
        a_state.old_hunks[0].op,
        conflictless2,
      );
      ...
      a_state.old_hunks[0] = {
        ...
        dlength: b_state.old_hunks[0].op.get_length_change(
          b_state.old_hunks[0].length,
        ),
        op: ar,
      };
      b_state.old_hunks[0] = {
        ...
        dlength: a_state.old_hunks[0].op.get_length_change(
          a_state.old_hunks[0].length,
        ),
        op: br,
      };
...
MAP.prototype.apply = function (document) {
  ...
  var d;
  if (typeof document == 'string') d = document.split(/.{0}/);
  // Clone array.
  else d = document.slice(); // clone
...
MAP.prototype.rebase_functions = [
  [
    ...
    function (other, conflictless) {
      ...
      if (!conflictless || !('document' in conflictless)) {
        ...
        var conflictless2 = clone(conflictless);
  ...
  [
    ...
    function (other, conflictless) {
      ...
      if (other.hunks.length == 0) return [this, other];
...
exports.createRandomOp = function (doc, context) {
  ...
  ops.push(function () {
    ...
    while (dx < doc.length) {
      ...
      var old_length =
        start < doc.length
          ? Math.floor(Math.random() * (doc.length - start + 1))
          : 0;
  ...
  ops.push(function () {
    while (true) {
      ...
      var random_elem;
      if (doc.length == 0) {
        if (typeof doc === 'string') random_elem = '?';
        else if (Array.isArray(doc)) random_elem = null;
      } else {
