...
function rebase_array(base, ops, conflictless, debug) {
  ...
  if (ops.length == 0 || base.length == 0) return ops;
  ...
  if (debug) {
    ...
    debug(
      ...
      'document' in conflictless ? JSON.stringify(conflictless.document) : '',
      '...',
    );
    var original_debug = debug;
    debug = function () {
      var args = ['>'].concat(Array.from(arguments));
      original_debug.apply(null, args);
    };
  }
  ...
  if (base.length == 1) {
    ...
    if (conflictless) {
      conflictless2 = clone(conflictless);
    ...
    // shallow clone
    conflictless = !conflictless ? null : clone(conflictless);
