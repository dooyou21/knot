...
// Own Modules...
import { Operation, NO_OP } from "./index";
...
export class LIST extends Operation {
  ...
  visit(visitor) {
    ...
    const ret = new LIST(this.ops.map(function(op) { return op.visit(visitor); }));
  ...
  internalFromJSON(json, protocol_version, op_map) {
    const ops = json.ops.map(function(op) {
      return super.opFromJSON(op, protocol_version, op_map);
    });
  ...
  atomic_compose(other) {
    ...
    // append
    const new_ops = this.ops.slice(); // clone
    new_ops.push(other);
    return new LIST(new_ops);
  }
  ...
  rebase(base, ops, conflictless, debug) {
    ...
    const ret = rebase_array(base, ops, conflictless, debug);
  ...
  drilldown(index_or_key) {
    return new LIST(this.ops.map(function(op) {
