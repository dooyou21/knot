...
test('random', function (tt) {
  for (var i = 0; i < 1000; i++) {
    ...
    var op1 = jot.createRandomOpSequence(
      initial_value,
      Math.floor(10 * Math.random()) + 1,
    );
    var op2 = jot.createRandomOpSequence(
      initial_value,
      Math.floor(10 * Math.random()) + 1,
    );
    ...
    tt.test(
      ...
      function (t) {
        ...
        var val1c = op1
          .compose(op2.rebase(op1, { document: initial_value }))
          .apply(initial_value);
        var val2c = op2
          .compose(op1.rebase(op2, { document: initial_value }))
          .apply(initial_value);
