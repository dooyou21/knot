...
import { PATCH, SPLICE, ATINDEX } from './sequences';
...
function _diff(a, b, options) {
  ...
  if (ta == 'string' && tb == 'string') return diff_strings(a, b, options);

  if (ta == 'array' && tb == 'array') return diff_arrays(a, b, options);

  if (ta == 'object' && tb == 'object') return diff_objects(a, b, options);
...
function diff_strings(a, b, options) {
  ...
  var method = 'Chars';
  if (options.words) method = 'Words';
  if (options.lines) method = 'Lines';
  if (options.sentences) method = 'Sentences';
  ...
  var op = new PATCH(hunks).simplify();
  return {
    op: op,
...
function diff_arrays(a, b, options) {
  ...
  var ai = a.map(function (item, i) {
    return i;
  });
  var bi = b.map(function (item, i) {
    return i;
  });
  ...
  function do_diff(ai, bi, level) {
    ...
    generic_diff(ai, bi, function (ai, bi) {
      ...
      if (!change.removed && !change.added) {
        ...
        hunks.push({
          type: 'equal',
          ai: ai.slice(a_index, a_index + change.items.length),
          bi: bi.slice(b_index, b_index + change.items.length),
        });
        ...
        if (change.added) {
          // Added.
          hunks[hunks.length - 1].bi = hunks[hunks.length - 1].bi.concat(
            change.items,
          );
          b_index += change.items.length;
        } else if (change.removed) {
          // Removed.
          hunks[hunks.length - 1].ai = hunks[hunks.length - 1].ai.concat(
            change.items,
          );
          a_index += change.items.length;
        }
    ...
    hunks.forEach(function (hunk) {
      ...
      if (hunk.ai.length != hunk.bi.length) {
        ...
        for (var i = 0; i < hunk.ai.length; i++) {
          ...
          // Add an operation.
          if (!d.op.isNoOp()) ops.push(new ATINDEX(hunk.bi[i], d.op));
...
function diff_objects(a, b, options) {
  ...
  for (var key in a) {
    if (key in b) {
      // Compute diff.
      let d = _diff(a[key], b[key], options);

      // Add operation if there were any changes.
      if (!d.op.isNoOp()) ops.push(new APPLY(key, d.op));
  ...
  pairs.forEach(function (item) {
    ...
    // Use this pair.
    ops.push(new REM(item.a_key, item.b_key));
    if (!item.diff.op.isNoOp()) ops.push(new APPLY(item.b_key, item.diff.op));
  ...
  for (var key in a) {
    ...
    ops.push(new REM(key));
  }
  for (var key in b) {
    if (key in a || key in used_b) continue;
    ops.push(new PUT(key, b[key]));
  }
