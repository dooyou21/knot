...
COPY.prototype.inspect = function (depth) {
  return util.format(
    '<COPY %s>',
    this.pathpairs
      .map(function (pathpair) {
        return (
          serialize_pointer(pathpair[0]) +
          ' => ' +
          serialize_pointer(pathpair[1])
        );
      })
      .join(', '),
  );
...
function parse_path(jp, document) {
  ...
  for (var i = 0; i < jp.length; i++) {
    var p = jp.path[i];
    if (Array.isArray(document)) p = parseInt(p);
    path.push(p);
    document = document[p];
  }
  return path;
...
function drilldown_op(jp, document, op) {
  ...
  for (var i = 0; i < path.length; i++) op = op.drilldown(path[i]);
  return op;
}
...
function wrap_op_in_path(jp, document, op) {
  ...
  var i = path.length - 1;
  while (i >= 0) {
    if (typeof path[i] == 'string') op = new APPLY(path[i], op);
    else op = new ATINDEX(path[i], op);
    i--;
  }
  return op;
}
...
COPY.prototype.inverse = function (document) {
  ...
  return new LIST(
    this.pathpairs.map(function (pathpair) {
      return wrap_op_in_path(
        pathpair[1],
        document,
        new SET(pathpair[1].get(document)),
      );
    }),
  );
};
...
function make_random_path(doc) {
  ...
  if (typeof doc === 'string' || Array.isArray(doc)) {
    ...
    path.push('' + idx);
    try {
      if (Math.random() < 0.5 && typeof doc !== 'string')
    ...
    path.push(key);
    try {
      if (Math.random() < 0.5) path = path.concat(make_random_path(doc[key]));
    } catch (e) {
      // ignore - can't create path on inner value
    }
  } else {
